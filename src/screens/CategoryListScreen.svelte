<script lang="ts">
	...
	async function handleDelete() {
		if (categoryToDelete) {
			...
			const count = await db.items.where('category').equals(categoryToDelete.id).count();
			if (count > 0) {
				alert(`Cannot delete category "${categoryToDelete.name}" because it is used by ${count} expense(s). Please reassign them first.`);
			} else {
...
{#snippet leftIcon()}
	<button 
		...
		class="text-discord-text-normal hover:text-white transition-colors p-1"
...
<div class="h-full flex flex-col relative pb-20">
	...
	<div class="flex-1 overflow-y-auto p-4 space-y-3">
		<p class="text-[13px] text-discord-text-muted px-1">
			...
			{#each categories as category (category.id)}
				<Card variant="sidebar" padding="none" class="flex items-center gap-3 px-3 py-3">
					...
					<div class="flex-1 min-w-0">
						<div class="font-bold text-white text-[15px]">{category.name}</div>
					</div>
					<div class="flex items-center gap-2">
						<button 
							onclick={() => goto(resolve(`/categories/${category.id}/edit`))}
							class="p-2 text-discord-text-muted hover:text-white transition-colors"
						...
						<button 
							...
							class="p-2 text-discord-text-muted hover:text-discord-danger transition-colors"
	...
	<div class="fixed bottom-6 right-6 z-50">
		<button 
			...
			class="w-14 h-14 bg-discord-blurple hover:bg-[#4752C4] text-white rounded-full flex items-center justify-center shadow-lg transition-transform hover:scale-105 active:scale-95"
...
>
	<div class="space-y-4">
		<p class="text-discord-text-normal">
			Are you sure you want to delete the category <span class="text-white font-bold">{categoryToDelete?.name}</span>? This action cannot be undone.
		</p>
